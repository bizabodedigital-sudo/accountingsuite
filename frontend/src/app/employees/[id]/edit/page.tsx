...
export default function EditEmployeePage() {
  ...
  const loadEmployee = async () => {
    try {
      ...
      setDateOfHire(emp.dateOfHire ? new Date(emp.dateOfHire).toISOString().split('T')[0] : '');
      setDateOfTermination(emp.dateOfTermination ? new Date(emp.dateOfTermination).toISOString().split('T')[0] : '');
      setEmploymentType(emp.employmentType || 'FULL_TIME');
      setDepartment(emp.department || '');
      setPosition(emp.position || '');
      setBaseSalary(emp.baseSalary || '');
      setSalaryFrequency(emp.salaryFrequency || 'MONTHLY');
      setHourlyRate(emp.hourlyRate || '');
      setOvertimeRate(emp.overtimeRate || '');
      setTrn(emp.trn || '');
      setNisNumber(emp.nisNumber || '');
      setBankName(emp.bankName || '');
      setBankAccountNumber(emp.bankAccountNumber || '');
      setBankAccountType(emp.bankAccountType || 'CHECKING');
      ...
      if (emp.address) {
        ...
        setParish(emp.address.parish || '');
        setPostalCode(emp.address.postalCode || '');
      }
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await employeeAPI.updateEmployee(employeeId, {
        ...
        dateOfHire,
        dateOfTermination: dateOfTermination || undefined,
        employmentType,
        department: department || undefined,
        position: position || undefined,
        baseSalary: typeof baseSalary === 'number' ? baseSalary : parseFloat(String(baseSalary)) || 0,
        salaryFrequency,
        hourlyRate: typeof hourlyRate === 'number' ? hourlyRate : (hourlyRate ? parseFloat(String(hourlyRate)) : undefined),
        overtimeRate: typeof overtimeRate === 'number' ? overtimeRate : parseFloat(String(overtimeRate)) || 0,
        trn: trn || undefined,
  ...
  return (
    <div className="h-full flex flex-col bg-gray-50 dark:bg-gray-900">
      <Header 
        onMenuClick={() => {}}
        title="Edit Employee"
        subtitle={`Editing ${firstName} ${lastName}`}
      />
